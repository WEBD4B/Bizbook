...
export function FinancialOverviewChart({ 
  ...
  const totalCreditLimit = creditCards.reduce((sum, card) => sum + parseFloat(card.creditLimit), 0);
  const totalCreditUsed = creditCards.reduce((sum, card) => sum + parseFloat(card.balance), 0);
  ...
  const debtData = [
    ...creditCards.map((card, index) => ({
      ...
      type: "Credit Card",
      utilization: parseFloat(card.balance) / parseFloat(card.creditLimit) * 100
    })),
  ...
  const cashFlowData = [
    ...
    { name: "Debt Payments", value: [...creditCards, ...loans].reduce((sum, item) => sum + parseFloat('minimumPayment' in item ? item.minimumPayment : item.monthlyPayment), 0), color: COLORS[3] },
